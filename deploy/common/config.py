...
def joint_indices(config: dict[str, Any], key: str) -> NDArray[np.int64]:
    """Read and validate a joint-index permutation."""
    values = np.asarray(config[key], dtype=np.int64)
    if values.shape != (_JOINT_COUNT,) or not np.array_equal(np.sort(values), np.arange(_JOINT_COUNT)):
        raise ValueError(f"{key} must be a permutation of 0 to {_JOINT_COUNT - 1}")
    return values
