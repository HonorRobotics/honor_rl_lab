...
def to_policy_order(
    ...
    joint2motor_idx: NDArray[np.int64],
) -> NDArray[np.float32]:
    """Reorder a motor vector for policy input."""
    return values[joint2motor_idx]
...
def to_motor_order(
    ...
    """Reorder a policy vector for motor output."""
    reordered = np.empty_like(values)
    reordered[joint2motor_idx] = values
    return reordered
